...
function buildOptions() {
  options.kodi_ip = (localStorage.getItem("kodi_ip") || "").trim();
  options.kodi_user = localStorage.getItem("kodi_user") || "";
  options.kodi_password = localStorage.getItem("kodi_password") || "";
  options.kodi_auth =
    options.kodi_user && options.kodi_password
      ? Base64.encode(options.kodi_user + ":" + options.kodi_password)
      : "";
}

function sendRequest(method, params, callback) {
  if (!options.kodi_ip) {
    if (callback) callback("No IP configured", null);
    return;
  }
  ...
  if (options.kodi_auth) {
    req.setRequestHeader("Authorization", "Basic " + options.kodi_auth);
  }
...
var sendCurrentStatus = function (data, data2, data3, data4) {
  ...
  dict.playback_main =
    data3.result.item.type === "episode"
      ? data3.result.item.showtitle
      : data3.result.item.label;
  dict.playback_sub =
    data3.result.item.type === "episode"
      ? data3.result.item.season.toString() +
        "x" +
        data3.result.item.episode.toString() +
        " " +
        data3.result.item.title
      : data3.result.item.type === "song"
        ? data3.result.item.artist[0]
        : data3.result.item.type === "channel"
          ? data3.result.item.title
          : "";
  if (data4.result) {
    dict.playback_status = "Playing";
    dict.playback_elapsed =
      (data4.result.time.hours ? data4.result.time.hours + ":" : "") +
      pad(data4.result.time.minutes, 2) +
      ":" +
      pad(data4.result.time.seconds, 2);
    if (data4.result.totaltime.minutes || data4.result.totaltime.seconds) {
      dict.playback_elapsed +=
        " / " +
        (data4.result.totaltime.hours
          ? data4.result.totaltime.hours + ":"
          : "") +
        pad(data4.result.totaltime.minutes, 2) +
        ":" +
        pad(data4.result.totaltime.seconds, 2);
    }
    dict.input_title =
      data3.result.item.type === "episode"
        ? "TV Show"
        ...
        sendRequest("Player.GetActivePlayers", {}, function (err2, data2) {
          if (data2 && data2.result.length > 0) {
            ...
            sendCurrentStatus(
              data,
              data2,
              { result: { item: { label: "Nothing playing", type: "N/A" } } },
              {}
            );
          }
        });
      ...
      if (data) {
        ...
        for (var i = 0; i < dict.nb_items; i++) {
          dict[(10000 + i * 10 + 0).toString()] =
            data.result.episodes[i].episodeid;
          dict[(10000 + i * 10 + 1).toString()] =
            data.result.episodes[i].showtitle;
    ...
    function (err, data) {
      if (data) {
        ...
        for (var i = 0; i < data.result.tvshows.length; i++) {
          var unwatched =
            data.result.tvshows[i].episode -
            data.result.tvshows[i].watchedepisodes;
          if (unwatchedonly && unwatched === 0) {
            continue;
          }
          dict[(10000 + nb_items * 10 + 0).toString()] =
            data.result.tvshows[i].tvshowid;
          dict[(10000 + nb_items * 10 + 1).toString()] =
            data.result.tvshows[i].label;
          if (unwatchedonly) {
            dict[(10000 + nb_items * 10 + 2).toString()] =
              unwatched +
              " episode" +
              (unwatched === 1 ? "" : "s") +
              " to watch";
            ...
            dict[(10000 + nb_items * 10 + 2).toString()] =
              data.result.tvshows[i].season.toString() +
              (data.result.tvshows[i].season === 1
                ? " season, "
                : " seasons, ") +
    ...
    function (err, data) {
      if (data) {
        ...
        for (var i = 0; i < dict.nb_items; i++) {
          ...
          dict[(10000 + i * 10 + 1).toString()] =
            "Season " + data.result.seasons[i].season.toString();
          var unwatched =
            data.result.seasons[i].episode -
            data.result.seasons[i].watchedepisodes;
          if (unwatched === 0) {
            dict[(10000 + i * 10 + 2).toString()] =
              data.result.seasons[i].episode.toString() +
              " episode" +
    ...
    function (err, data) {
      if (data) {
        ...
        for (var i = 0; i < dict.nb_items; i++) {
          dict[(10000 + i * 10 + 0).toString()] =
            data.result.episodes[i].episodeid;
          dict[(10000 + i * 10 + 1).toString()] = data.result.episodes[
            i
          ].label.substring(0, 20);
          dict[(10000 + i * 10 + 2).toString()] =
            data.result.episodes[i].firstaired.toString() +
    ...
    function (err, data) {
      if (data) {
        ...
        for (var i = 0; i < dict.nb_items; i++) {
          ...
          dict[(10000 + i * 10 + 2).toString()] =
            data.result.movies[i].year.toString() +
            (data.result.movies[i].rating
              ? ", " +
                (
                  Math.round(data.result.movies[i].rating * 10) / 10
                ).toString() +
                "/10"
              : "");
        }
  ...
  sendRequest(
    ...
    function (err, data) {
      if (data) {
        ...
        for (var i = 0; i < dict.nb_items; i++) {
          dict[(10000 + i * 10 + 0).toString()] =
            data.result.channelgroups[i].channelgroupid;
          dict[(10000 + i * 10 + 1).toString()] =
            data.result.channelgroups[i].label;
        }
        console.log(JSON.stringify(dict));
        send(dict);
      } else {
...
var sendKeypress = function (key) {
  sendRequest("Input." + key, {}, function (err, data) {
    if (data) {
      console.log("Keypress sent: " + key);
    }
    if (err) {
      console.log("Keypress not sent: " + key);
    }
  });
};
    ...
    switch (e.payload.data_request) {
      ...
      case "episodes":
        if (e.payload.data_request_id && e.payload.data_request_id2) {
          getEpisodes(
            e.payload.data_request_id,
            e.payload.data_request_id2,
            {}
          );
        }
        break;
...
Pebble.addEventListener("webviewclosed", function (e) {
  if (e.response) {
    ...
    if (stored.kodi_ip !== undefined) {
      localStorage.setItem("kodi_ip", stored.kodi_ip);
    }
    if (stored.kodi_user !== undefined) {
      localStorage.setItem("kodi_user", stored.kodi_user);
    }
    if (stored.kodi_password !== undefined) {
      localStorage.setItem("kodi_password", stored.kodi_password);
    }
    buildOptions();
    if (options.kodi_ip) {
      getBasicInfo();
